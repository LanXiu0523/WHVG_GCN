...
class GraphCNN(tf.keras.Model):
    ...
    def __preprocess_neighbors_sumavepool(self, batch_graph):
        ...
        Adj_block_idx = tf.cast(tf.transpose(Adj_block_idx),tf.int64)#int64

        #For edge weighting between HVG nodes
        #The weight is the reciprocal of the index difference between the two nodes
        Adj_block_idx_numpy = Adj_block_idx.numpy()
        Adj_block_weight = numpy.zeros(len(Adj_block_idx_numpy))
        for Edge_index in range(len(Adj_block_idx_numpy)):
            Adj_block_weight[Edge_index] = (float(1/(abs(Adj_block_idx_numpy[Edge_index][0]-Adj_block_idx_numpy[Edge_index][1]))))
        Adj_block_weight = tf.convert_to_tensor(Adj_block_weight)
        
        Adj_block = tf.SparseTensor(indices=Adj_block_idx, values=Adj_block_weight, dense_shape=[start_idx[-1],start_idx[-1]])        
        return Adj_block
    ...
    def next_layer_eps(self, h, layer, padded_neighbor_list = None, Adj_block = None):
        ...
        if self.neighbor_pooling_type == "max":
            ...
            Adj_block = tf.cast(Adj_block,tf.float32)
        ...
        '''
        Hv = self.mlps[layer](pooled)
        Hu_weight_SUM = self.mlps[layer]((1+self.eps[layer])*h2)
        pooled_rep = Hv + Hu_weight_SUM
